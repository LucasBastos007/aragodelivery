...
async function uploadFoto(file: File, lojaId: string, produtoId: string): Promise<{ url: string; erro?: string }> {
  try {
...
export default function AdminCardapioPage() {
  ...
  async function carregarNomeLoja() {
    const { data } = await supabase.from("lojas").select("nome").eq("id", lojaId).single()
    if (data?.nome) setNomeLoja(data.nome)
  }

  async function carregarTudo() {
    setLoading(true)
    const [catRes, prodRes] = await Promise.all([
      fetch(`/api/admin/loja/${lojaId}/categorias`, { credentials: "include" }),
      fetch(`/api/admin/loja/${lojaId}/produtos`, { credentials: "include" }),
    ])
  ...
  async function salvarCategoria() {
    ...
    setSalvando(true)
    setErroSalvar("")
    const res = await fetch(`/api/admin/loja/${lojaId}/categorias`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ nome: nomeCat.trim(), ordem: categorias.length }),
    })
    if (!res.ok) {
  ...
  async function deletarCategoria(id: string) {
    ...
    await fetch(`/api/admin/loja/${lojaId}/categorias`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  async function salvarProduto() {
    if (!formProd.nome.trim() || !formProd.preco) return
    setSalvando(true)
    setErroSalvar("")
    ...
    if (fotoFile) {
      const produtoIdParaUpload = editando?.id ?? uid()
      const { url, erro } = await uploadFoto(fotoFile, lojaId, produtoIdParaUpload)
      if (!url) {
  ...
  async function toggleDisponivel(p: Produto) {
    await fetch(`/api/admin/loja/${lojaId}/produtos`, {
      ...
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ id: p.id, disponivel: !p.disponivel }),
    })
    await carregarTudo()
  }
  ...
  return (
    <div style={{ padding: "20px 16px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, fontSize: 13, color: "#94a3b8" }}>
        ...
        <span>/</span>
        <span style={{ color: "#64748b", fontWeight: 600 }}>{nomeLoja || lojaId}</span>
        <span>/</span>
        <span style={{ color: "#0F172A", fontWeight: 700 }}>Cardápio</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: "40px 20px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 20 }}>
            Crie categorias (ex: Lanches, Bebidas) e adicione produtos.
          </p>
