...
export default function LojaCuponsPage() {
  ...
  async function handleCriar() {
    ...
    if (error) { setErro(error.message.includes("unique") ? "Esse código já existe." : error.message); setSalvando(false); return }
    setCodigo(""); setValor(""); setMinimo(""); setValidade(""); setTipo("percentual")
    setCriando(false); setSalvando(false)
    carregar()
  }
  ...
  return (
    <div style={{ padding: "32px 36px", maxWidth: 780 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ color: "white", fontWeight: 900, fontSize: 22 }}>🎟️ Cupons</h1>
          <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 13, marginTop: 3 }}>Desconto exclusivo para sua loja</p>
        </div>
        <button
      ...
      {criando && (
        <div style={{ background: "#111", borderRadius: 16, border: "1px solid rgba(249,115,22,0.2)", padding: "20px 22px", marginBottom: 24, display: "flex", flexDirection: "column", gap: 14 }}>
          <p style={{ color: "white", fontWeight: 700, fontSize: 14 }}>Novo cupom</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={{ display: "block", color: "rgba(255,255,255,0.4)", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Código *</label>
              <input style={inp} value={codigo} onChange={e => setCodigo(e.target.value.toUpperCase())} placeholder="PROMO10" />
            </div>
            <div>
              <label style={{ display: "block", color: "rgba(255,255,255,0.4)", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Tipo</label>
              <div style={{ display: "flex", gap: 8 }}>
                {(["percentual", "fixo"] as Tipo[]).map(t => (
                  <button key={t} onClick={() => setTipo(t)} style={{
                    ...
                    background: tipo === t ? "rgba(249,115,22,0.15)" : "rgba(255,255,255,0.05)",
                    color: tipo === t ? "#f97316" : "rgba(255,255,255,0.4)",
            ...
            <div>
              <label style={{ display: "block", color: "rgba(255,255,255,0.4)", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Pedido mínimo (R$)</label>
              <input style={inp} type="number" min="0" value={minimo} onChange={e => setMinimo(e.target.value)} placeholder="0 = sem mínimo" />
            </div>
            <div>
              <label style={{ display: "block", color: "rgba(255,255,255,0.4)", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Válido até</label>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", marginTop: 48 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.4)", fontWeight: 600 }}>Nenhum cupom criado</p>
          <p style={{ color: "rgba(255,255,255,0.2)", fontSize: 13, marginTop: 4 }}>Crie cupons de desconto para atrair clientes</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {cupons.map(c => (
            <div key={c.id} style={{
              ...
              border: `1px solid ${c.ativo ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.03)"}`,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                  ...
                  <span style={{
                    ...
                    background: c.ativo ? "rgba(34,197,94,0.12)" : "rgba(255,255,255,0.05)",
                    color: c.ativo ? "#22c55e" : "rgba(255,255,255,0.3)",
                  }}>
              ...
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                <button onClick={() => toggleAtivo(c)} style={{
                  ...
                  background: "transparent", color: "rgba(255,255,255,0.4)", fontSize: 12, fontWeight: 700, cursor: "pointer",
                }}>
