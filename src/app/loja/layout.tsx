...
const NAV = [
  { href: "/loja",            icon: "📦", label: "Pedidos" },
  { href: "/loja/dashboard",  icon: "📊", label: "Dashboard" },
  { href: "/loja/cardapio",   icon: "🍽️", label: "Cardápio" },
  { href: "/loja/cupons",     icon: "🎟️", label: "Cupons" },
  { href: "/loja/financeiro", icon: "💰", label: "Financeiro" },
  { href: "/loja/perfil",     icon: "⚙️", label: "Minha loja" },
]
...
export default function LojaLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      <aside style={{ width: 210, background: "#0d0d0d", borderRight: "1px solid #1a1a1a", flexShrink: 0 }}
        className="flex flex-col">
        <div className="p-5 pb-4" style={{ borderBottom: "1px solid #1a1a1a" }}>
          <p className="font-black text-lg" style={{ color: "#f97316" }}>🛵 Arago</p>
          <p className="text-xs font-semibold truncate" style={{ color: "rgba(255,255,255,0.25)", marginTop: 2 }}>
            {sessao.loja_nome}
          </p>
        </div>
        <nav className="flex flex-col gap-1 p-3 flex-1">
          {NAV.map(n => {
            const active = n.href === "/loja" ? path === "/loja" : path.startsWith(n.href)
            return (
        ...
        <div className="p-4 flex flex-col gap-2" style={{ borderTop: "1px solid #1a1a1a" }}>
          <Link href="/" className="text-xs font-semibold" style={{ color: "rgba(255,255,255,0.2)", textDecoration: "none" }}>
            ← Voltar ao site
          </Link>
